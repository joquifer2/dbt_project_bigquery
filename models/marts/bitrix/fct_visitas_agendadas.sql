{{ config(
    materialized='table',
    schema='marts_bitrix',
    alias='fct_visitas_agendadas',
    partition_by={'field': 'date_first_visit', 'data_type': 'date'},
    cluster_by=['region', 'channel'],
    meta={'description': 'Tabla de hechos que registra todas las visitas agendadas por comercial, con atributos temporales y de dimensión. Si falta date_first_visit, se usa date_create_deal.'}
) }}

-- =============================================================================
-- Modelo: fct_visitas_agendadas
-- Descripción: Tabla de hechos que registra todas las visitas agendadas por comercial, enriquecida con dimensiones temporales, geográficas, comerciales y de canal.
-- Si falta date_first_visit, se usa date_create_deal.
-- =============================================================================

WITH
  -- Visitas agendadas base
  visitas AS (
    SELECT
      date_first_visit,           -- Fecha de la visita agendada (o creación del deal si falta)
    ...
    FROM {{ ref('int_visitas_agendadas') }}
  ),

  -- Dimensión temporal: fechas de la visita agendada
  fechas AS (
    SELECT
      date             AS date_first_visit,   -- Fecha de la visita agendada
...
-- =============================================================================
-- SELECT final: une las visitas agendadas con todas las dimensiones y fechas
-- =============================================================================
SELECT
  v.date_first_visit,                -- Fecha de la visita agendada
  ...
  v.date_closed_lead,                -- Fecha de cierre del lead
  ...
  cn.medium,                         -- Medio
  v.total_amount                     -- Monto total del deal
FROM visitas v
LEFT JOIN fechas f
